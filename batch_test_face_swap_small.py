#!/usr/bin/env python3
"""
Small batch test: 3 source × 3 target = 9 face swap tests
"""
...
def run_batch_tests():
    """Run small batch face swap tests"""
    API_KEY = load_api_key()
    if not API_KEY:
        print("Error: Please set REACT_APP_SEGMIND_API_KEY in .env file")
        return
    
    # Get source and target images (first 3 of each)
    source_images = sorted(glob.glob("test-results/source-images/source_*.jpg"))[:3]
    target_images = sorted(glob.glob("test-results/target-images/target_*.jpg"))[:3]
    ...
    if not source_images or not target_images:
        print("❌ No source or target images found. Run generate_test_images_small.py first.")
        return
...
def main():
    """Main function"""
    print("🧪 Small Batch Face Swap Test")
    print("=============================")
    print("Testing 3×3 = 9 combinations")
